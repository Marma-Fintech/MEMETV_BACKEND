  ...
  try {
    ...
    if (
      ...
      if (dailyReward) {
        ...
        logger.info(
          `Updated existing daily reward for telegramId: ${telegramId} with reward amount: ${rewardAmount}`
        )
      } else {
        ...
        logger.info(
          `Created new daily reward entry for telegramId: ${telegramId} with reward amount: ${rewardAmount}`
        )
      }
    ...
    logger.error(
      `Error while claiming Login Streak Reward for telegramId: ${telegramId}, index: ${index}. Error: ${err.message}`
    )
    next(err)
  }
}
...
const streakOfStreakRewardClaim = async (req, res, next) => {
  try {
    ...
    if (user.streak.multiStreak.streakOfStreakRewards.length > 0) {
      logger.info(
        `Streak of Streak Rewards available for telegramId: ${telegramId}`
      )

      let rewardAmount = 0
      user.streak.multiStreak.lastSOSReward =
        user.streak.multiStreak.streakOfStreakRewards[
          user.streak.multiStreak.streakOfStreakRewards.length - 1
        ]
      ...
      logger.warn(
        `No Streak of Streak rewards to claim for telegramId: ${telegramId}`
      )
      res
        .status(400)
